...
### CLI wiring (`@mdpact/cli`)
`mdpact test` now composes `createDriftRules` alongside `createBehaviorRules` so both fire against the spec after a behavior run.
