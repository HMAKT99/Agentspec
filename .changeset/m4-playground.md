...
- Runs the full lint pipeline (`@mdpact/core` + every built-in rule from `@mdpact/rules`) directly in the browser; no network calls, no uploads.
- Score panel uses the same scoring deductions as `mdpact score` (errors −8, warnings −3, info −1, floored at 0).
